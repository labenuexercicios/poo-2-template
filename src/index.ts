...
app.get("/accounts", async (req: Request, res: Response) => {
    try {
        ...
        const accountsDB = await accountDatabase.getAccounts()
...
app.put("/accounts/:id/balance", async (req: Request, res: Response) => {
    try {
        ...
        if (typeof value !== "number") {
            res.status(400).send("'value' deve ser number");
            return;
        }
        ...
        const newBalance = accountDBExists.balance + value;

        await accountDatabase.updateAccountBalance(id, newBalance);

        accountDBExists.balance = newBalance;

        res.status(200).send(accountDBExists);
    } catch (error) {
        console.error(error);
